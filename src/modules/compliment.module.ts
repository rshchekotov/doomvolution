...
const triggers = {
    "positive": [
        'genius', 'smart', 'intelligent',
        'charming', 'epic', 'legendary',
        'humble', 'sweet', 'lovely', 'great',
        'krista', 'talented', 'boppin', 
        'awesome', 'nice', 'perfect', 'lit'
    ],
    "negative": [
        'sad', 'depressing', 'frustrating',
        'infuriating', 'angry', 'shit',
        'rip', 'annoying', 'pissed',
        'screwed', 'cheesed', 'annoyed'
    ]
};

//TODO: Add Resources!
const responses = {
    'you': ['https://media1.tenor.com/images/3ca8f884da9396c1a23864cfcaf087e6/tenor.gif'],        // Youuuuu
    'alright': [
...
export class ComplimentModule extends Module {
    ...
    run = async (event: string, data: any, config: GuildConfig) => {
        ...
        if((match = /^who(?: is|'s) (?:a )?(.+)\??/.exec(data.content.toLowerCase())) != null) {
            if(triggers.positive.includes(match[1].split('\W')[0]))
                send(data.channel_id, choose(responses.you)!);
        } else if(triggers.negative.some(n => data.content.toLowerCase().includes(n))) {
