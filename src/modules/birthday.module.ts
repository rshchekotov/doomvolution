...
async function congratulate(config: GuildConfig, id: string) {
    ...
    let embed = new MessageEmbed()
        ...
        .setTimestamp()
        .setThumbnail('https://www.positivelyosceola.com/wp-content/uploads/2017/06/celebrationfireworks.jpg')
        .setFooter(`For ${user.nickname || user.user.username} | Brought to you by everyone!`);
...
export class BirthdayModule extends Module {
    ...
    load(config: GuildConfig) {
        ...
        if(config.data.birthdays) {
            for(let birthday in config.data.birthdays) {
                ...
                this.crontasks.push(cron.schedule(`0 */9 ${date.getDate()} ${date.getMonth()+1} *`, () => {
    ...
    verify = async (event: string, data: any, config: GuildConfig) => {
        return event === this.types[0] || await this.cmd(data, this.re, config) !== null;
    };
