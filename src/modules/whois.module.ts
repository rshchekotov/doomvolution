...
import { getAvatar, getMember, getMessage, getUser, send } from "@/util/discord.util";
...
export class WhoIsModule extends Module {
    ...
    'You can do so, by simply typing: '
        ...
        if(aliases.includes(match![1])) {
            ...
            let embed = new MessageEmbed()
                ...
                .addFields([
                    ...
                    { name: 'Bot', value: (user.user.bot || user.user.system) ? 'Yep' : 'Nope' },
                    { name: '\u200B', value: '\u200B' },
                    { name: 'Exists Since', value: new Date(user.user.createdTimestamp).toLocaleString() },
