...
export class TutorialModule extends Module {
    ...
    verify = async (event: string, data: any, config: GuildConfig) => {
        return event === this.types[1] || (await this.cmd(data, /^tutorial$/, config)) != null;
    };
    ...
    run = async (event: string, data: any, config: GuildConfig) => {
        if(event === this.types[0]) {
            ...
            let embed: MessageEmbed = new MessageEmbed()
                ...
                .setColor('#009900')
                .setThumbnail('https://hocarm.org/content/images/2020/04/Tutorial-Graphic_Web-04-1.png')
            ...
            try {
                msg = await getMessage(data.channel_id, data.message_id);
            } catch {
