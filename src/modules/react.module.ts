    ...
    run = async (event: string, data: any, config: GuildConfig) => {
        ...
        if(react.includes(keyword)) {
            ...
            if(structure.includes(content)) {
                ...
                let print = (object: any) => {
                    Object.keys(object).forEach(key => {
                        ...
                        response += ('\t'.repeat(indentation) + `${key} ${l ? `(${l})` : '{'}\n`);
                        if(!object[key][0]) {
                            indentation++;
                            print(object[key]);
                            indentation--;
            ...
            if(insert) {
                ...
                guild.config.data.react = create(config.data.react, insert[1].split(/[ >./]+/), insert[2]);
                guild.pushConfig();
                (await getMessage(data.channel_id, data.id)).react('✅');
            } else if(show) {
                let options: string[] = getArray(config.data.react, show[1].split(/[ >./]+/));
                if(options.length > 0) {
