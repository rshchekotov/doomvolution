...
export class ManualModule extends Module {
  ...
  run = async (event: string, data: any, config: GuildConfig) => {
    if (event === this.types[1]) {
      ...
      if (
        !embed.title ||
        (!embed.title!.startsWith('Man') && !embed.title!.startsWith('WoMan'))
      ) {
        return;
      } // Skip if not Man Entry
      ...
      if (!this.cache[data.channel_id]) {
        ...
        if (!page) {
          return;
        } // Skip if One-Page Entry / Not Found
      }
      ...
      let resp: MessageEmbed = new MessageEmbed()
        .setTitle(
          mod
            ? `${match![0].startsWith('wo') ? 'WoMan' : 'Man'} | ${mod.name}`
            : `Man | No Results for '${match![1]}'`
        )
