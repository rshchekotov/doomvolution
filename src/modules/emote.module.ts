...
export class EmoteModule extends Module {
  ...
  man: string =
    ...
    '> *You can get free nitro.* What does it cost? Your humanity.\n' +
    'by Kami-chan.';
  ...
  run = async (event: string, data: any, config: GuildConfig) => {
    let create = /emote <(a?):([^:]+):(\d+)>/.exec(
      data.content.substring(config.prefix.length)
    );
    ...
    if (create) {
      try {
        ...
        await server.emojis.create(
          `https://cdn.discordapp.com/emojis/${create[3]}.${
            create[1] ? 'gif' : 'png'
          }`,
          create[2]
        );
      ...
      await sendAsHook(
        data.channel_id,
        await hookify(await getMember(config.gid, data.author.id)),
        content
      );
