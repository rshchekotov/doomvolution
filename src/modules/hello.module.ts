...
export class HelloModule extends Module {
  ...
  types: string[] = ['MESSAGE_CREATE', 'GUILD_MEMBER_ADD'];
  requires: string[] = [];
  man: string =
    '' +
    '***__Hello Module__***\n' +
    ...
    'to the welcome / general bot channel, then every new guild ' +
    'member will receive that message. Note when you register that ' +
  ...
  verify = async (event: string, data: any, config: GuildConfig) => {
    return (
      event === this.types[1] || (await this.cmd(data, this.re, config)) != null
    );
  };

  run = async (event: string, data: any, config: GuildConfig) => {
    if (event === this.types[1]) {
      if (config.data.channel && config.data.welcome) {
        await send(config.data.channel, config.data.welcome);
      }
    } else if (aliases.includes((await this.cmd(data, this.re, config))![1])) {
      ...
      let embed = new MessageEmbed()
        ...
        .setFooter(
          `from ${initial.member!.nickname || initial.author.username}`
        )
        .setTimestamp();
      ...
      if (initial.mentions && initial.mentions.users.array().length > 0)
        await send(
          data.channel_id,
          initial.mentions.users.map((user) => `<@!${user.id}>`).join(' ')
        );
    }
  };
}
