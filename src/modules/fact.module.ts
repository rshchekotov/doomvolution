...
export class FactModule extends Module {
  ...
  run = async (event: string, data: any, config: GuildConfig) => {
    ...
    if (match) {
      ...
      if (create.includes(keyword)) {
        ...
        if (body.fact && body.color && body.chance && body.rarity) {
          ...
          let msg = await send(
            data.channel_id,
            printFact(this.cache[fid]!, `f-${fid} | confirm`)
          );
          ...
          send(
            data.channel_id,
            'Missing one of required Properties: [fact, color, chance, rarity]'
          );
        }
      } else if (count.includes(keyword)) {
        send(
          data.channel_id,
          `Currently there are ${(config.data.facts || []).length} facts.`
        );
      }
    }
    ...
    if (this.counter % config.data.fact_frequency === 0) {
      ...
      let weighted: Array<WeightedObject<Fact>> = facts.map((fact) => {
        return { obj: fact, w: fact.chance };
      });
      let fact = weightedChoose(weighted)!;
      send(
        config.data.channel,
        printFact(fact, `${fact.rarity} Fact #${facts.indexOf(fact)}`)
      );
    }
  };
}
