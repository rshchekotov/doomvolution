...
export class PrefixModule extends Module {
    ...
    verify = async (event: string, data: any, config: GuildConfig) => {
        return data.content === config.prefix || 
    ...
    run = async (event: string, data: any, config: GuildConfig) => {
        ...
        if(data.content === config.prefix) {
            send(data.channel_id, handler.config.modules.map(mod => `- ${mod}`).join('\n'));
        } else {
            ...
            handler.pushConfig();
            await msg.react('🆗');
            await msg.reply(`Prefix successfully changed from '${config.prefix}' to ${handler.config.prefix}`);
        }
    };
