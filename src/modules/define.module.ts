...
export class DefineModule extends Module {
  ...
  run = async (event: string, data: any, config: GuildConfig) => {
    if (event === this.types[1]) {
      ...
      if (paged && msg.embeds && msg.embeds.length > 0) {
        if (left.includes(eid)) {
          paged.page =
            (paged.pages.length + paged.page - 1) % paged.pages.length;
        ...
        if (
          ...
          msg.embeds.length > 0 &&
          msg.embeds[0].title &&
          msg.embeds[0].title.startsWith('Definition')
        ) {
          await msg.reactions.removeAll();
    ...
    if (def.includes(match![1])) {
      ...
      let formatted: PagedEmbed | MessageEmbed = formatDefinitions(
        results || [],
        term
      );
