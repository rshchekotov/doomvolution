        ...
        if(event === this.types[0]) {
            if(!config.data.users) {
                ...
                guild.members.cache.forEach((member: GuildMember) => {
                    ...
                    if(!member.user.bot)
                        config.data.users[member.id].push('member');
                });
            ...
            if(role.includes(match![1])) {
                if(match![3]) {
                    ...
                    send(data.channel_id, `Successfully added ${match![3]} to <!@${match![2]}>`);
                } else {
                    send(data.channel_id, `User Roles for <@!${match![2]}>: [${config.data.users[match![2]].join(', ')}]`);
                }
            }
        }
    };
