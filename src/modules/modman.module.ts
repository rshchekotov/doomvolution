    ...
    if (event === this.types[0]) {
      ...
      if (install.includes(keyword)) {
        ...
        packages.forEach((pkg: string) => {
          ...
          if (!allowed) {
            ...
            send(
              data.channel_id,
              `Missing Following Dependencies/Settings: ${missing.join(', ')}`
            );
        ...
        send(
          data.channel_id,
          `Successfully Set: \`${confData[0]}\` to \`${dat}\``
        );
        ...
        packages.forEach((pkg: string) => {
          if (pkg === 'modman' || pkg === 'man') {
            send(
              data.channel_id,
              'To prevent damage, I will NOT remove ' +
                pkg +
          ...
          if (installed) {
            handler.config.modules = handler.config.modules.filter(
              (mod) => mod !== pkg
            );
            handler.pushConfig();
            ...
            send(
              data.channel_id,
              `Package '${pkg}' not installed. *nothing happened*`
            );
          }
        });
        ...
        let term: string | RegExp =
          research === null ? res![2] : new RegExp(research![1]);
        let found: string[] = [];
        ...
        for (let module in modules) {
          if (term instanceof RegExp && term.test(module)) {
            ...
            (typeof term === 'string' && module.includes(term)) ||
            term === 'all'
          ) {
            found.push(module);
          }
        ...
        if (found.length === 0) {
          ...
          for (let i = 0; i < found.length; i++) {
            let mod = `- ${found[i]}${
              config.modules.includes(found[i]) ? ' [installed]' : ''
            }\n`;
          ...
          let embed = new MessageEmbed()
            ...
            .setAuthor(
              `Search${
                found.length > 10
                  ? ` 1/${Math.floor(found.length / 10) + 1}`
                  : ''
              }`,
      ...
      if (search) {
        if (number.includes(eid)) {
          ...
          embed
            .setDescription(search.pages[number.indexOf(eid)])
            .setAuthor(
              `Search${
                search.pages.length > 1
                  ? ` ${search.page + 1}/${search.pages.length}`
                  : ''
              }`,
              'https://cdn.discordapp.com/emojis/789210400914145352.png'
            );
          ...
          await checkReactions(
            msg.channel.id,
            msg.id,
            ['❌'].concat(number.slice(0, search.pages.length))
          );
        } else if (eid === '❌') {
