...
export class ConfigBrowserModule extends Module {
  ...
  verify = async (event: string, data: any, config: GuildConfig) => {
    return (
      (event === this.types[1] && botDevs.includes(data.user_id)) ||
  ...
  run = async (event: string, data: any, config: GuildConfig) => {
    ...
    if (event === this.types[1]) {
      ...
      if (this.cache[msg.id]) {
        ...
        if (eid === '▶️') cso.page = (cso.page + 1) % pages;
        if (eid === '◀️') cso.page = (cso.page + pages - 1) % pages;
        if (eid === '⬆️') {
          cso = cd(cso, -1);
        }
        ...
        if (eid === '🟩') {
          await msg.channel.send(
            'Are you sure you want to save the current config?\nType `YES` to confirm.'
          );

          let collection = await msg.channel.awaitMessages(
            (m: Message) => {
              return (
                botDevs.includes(m.author.id) &&
          ...
          if (polled && polled.content === 'YES') {
            await msg.channel.send(
              `Succesfully deleted ${cso.path.slice(-1)[0]}!`
            );
            cso = rm(cso);
      ...
      try {
        ...
        for (
          let i = 0;
          i < Math.min(Object.keys(configuration).length, 5);
          i++
        ) {
