...
import { DefaultConfig, GuildConfig } from "@/interfaces/guild-config.interface";
...
export class GuildEventHandler {
    ...
    async pushConfig() {
        guilds.set(this.config.gid, this);
        await GuildConfigModel.replaceOne({ gid: this.config.gid }, this.config).exec();
    }
    ...
    setConfig(base: any, path: string[], cfg: any, append: boolean) {
        if(path.length > 0) {
            let field = path.shift() || 'q.q';
            base = Object.assign(base, this.setConfig(base[field], path, cfg, append));
        } else {
            if(append) {
