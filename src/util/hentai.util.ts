...
export function formatHentai(hentai: Hentai | undefined, search: string): MessageEmbed {
    if(!hentai)return new MessageEmbed()
        .setColor(0x990000)
        .setTitle(`Didn't find anything for ${search}`)
        .setTimestamp();
    return new MessageEmbed()
        .setColor(0x990000)
        .setTitle(`${hentai.id} | ${hentai.title.japanese} / ${hentai.title.english}`)
        .setURL(`https://nhentai.net/g/${hentai.id}/`)
        .addFields(
            { name: 'Tags', value: hentai.tags.map(tag => `(${tag.id} | ${tag.name})`).join(', ') },
